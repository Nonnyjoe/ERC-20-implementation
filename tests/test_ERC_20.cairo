...
fn test_transfer() {
    ...
    assert(ERC_20::balance_of(recipient) == transferAmount, 'Receiver ballance does not tally');
    assert(
...
fn test_grant_allowance() {
    ...
    ERC_20::grant_allowance(recipient, transferAmount);
    assert(check_user_allowance(sender, recipient) == Allowance_amount, 'Allowance Error');
}
